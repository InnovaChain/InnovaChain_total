...
const GenerationPromptStage = () => {
  const [selectedRecreationModal, setSelectedRecreationModal] =
    useState("Use as promopt");
  ...
  return (
    <div
      className={clsx(
        ...
        "text-[#979797] text-sm font-medium",
        className,
      )}
    >
      <p>{tabTitle}</p>
...
const PromptInputArea = ({
  ...
  return (
    <textarea
      className={clsx(
        "bg-[#F6F6F6] rounded-lg p-3",
        "text-[#8D8D8D] text-sm",
        "resize-none w-full min-h-[180px]",
        className,
      )}
    >
      {content}
